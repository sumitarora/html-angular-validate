...
var validateContent = function (content, callback, options) {
  w3cjs.setW3cCheckUrl('http://127.0.0.1:5000/w3cvalidatorservice');
    ...
    callback: function (res) {
      if (res.length === 0) {
        ...
        for (var i = 0; i < res.length; i += 1) {
          if (!checkRelaxed(res[i].message, options) &&
            !checkCustomTags(res[i].message, options) &&
            !checkCustomAttrs(res[i].message, options)) {
            errors.push({
              line: res[i].lastLine,
              col: res[i].lastColumn,
              msg: res[i].message
            });
          }
        }
...
module.exports = {
  vatidateContent: function (content, opts) {
    return new Promise(function (fullfill, reject) {
      validateContent(content, fullfill, opts);
    });
  },
